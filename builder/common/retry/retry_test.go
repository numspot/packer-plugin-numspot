...
func TestRetry(t *testing.T) {
	...
	err := Run(0, 0, 0, func(_ uint) (bool, error) {
		numTries++
		return true, nil
	})
	if numTries != 1 {
	...
	if err != nil {
		t.Fatalf(
			"Successful retried function should not have returned a retry error. Error: %s",
			err.Error(),
		)
	}
	...
	if !errors.Is(err, funcErr) {
		t.Fatalf(
			"Errant function did not return the right error %s. Error: %s",
			funcErr,
			err.Error(),
		)
	}
	...
	if numTries != expectedTries {
		t.Fatalf(
			"Unsuccessful retry function should have been called %d times. Only called %d times.",
			expectedTries,
			numTries,
		)
	}
	if !errors.Is(err, ErrRetryExhausted) {
		t.Fatalf(
			"Unsuccessful retry function should have returned a retry exhausted error. Actual error: %s",
			err.Error(),
		)
	}
}
